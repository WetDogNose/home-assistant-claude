...
- **`/config` is the user's home, not add-on scratch space.** `install_blueprint` writes the bundled blueprint exactly once and records a baseline copy in `/data`, so a later release can update a file nobody touched while leaving an edited one alone, and a file the user deleted is not recreated. It previously re-copied on every boot, which made the blueprint impossible to delete (it came back), impossible to harden (edits were reverted), and a source of untracked churn in any git repo kept over `/config`. Its path is deliberately not namespaced into a vendor subdirectory — automations reference a blueprint by path, so moving it would break every automation already built from it.
...
- **build-test.yml** (PRs touching `claude-terminal/**`) — hadolint + amd64/aarch64 builds, then amd64 smoke tests asserting specific scripts are executable, specific binaries exist, every bundled skill shipped with valid frontmatter, and the served `index.html` still contains both ttyd's client and the key bar. **Adding a script to `scripts/`, a skill to `skills/`, or a required binary means updating the lists in `ci/smoke.sh`.**
